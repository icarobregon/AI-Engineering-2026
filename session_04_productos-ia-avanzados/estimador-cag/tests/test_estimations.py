...
VALID_REQUEST = {
    "description": (
        "Cliente quiere una landing page con formulario de contacto, "
...
def test_estimate_missing_fields(client):
    response = client.post("/api/v1/estimate", json={"description": "x" * 30})
    assert response.status_code == 422


def test_estimate_description_too_short(client):
    response = client.post(
        "/api/v1/estimate",
        json={**VALID_REQUEST, "description": "short"},
    )
    assert response.status_code == 422


def test_estimate_invalid_enum(client):
    response = client.post(
        "/api/v1/estimate",
        json={**VALID_REQUEST, "project_type": "spaceship"},
    )
    assert response.status_code == 422
