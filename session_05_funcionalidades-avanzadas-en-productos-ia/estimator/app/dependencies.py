...
def get_session_store() -> SessionStore:
    ...
    Not wrapped in ``@lru_cache`` because ``lru_cache`` on a plain function
    returning a mutable singleton can cause test-isolation issues.  The store
    is a module-level singleton instead.
    """
    return _session_store


def get_conversation_service() -> ConversationService:
    """Return a ConversationService wired to the shared LLMWrapper."""
...
_session_store = SessionStore(max_turns=get_settings().MAX_TURNS)
