...
class LLMWrapper:
    ...
    def complete_structured(
        ...
        *,
        ...
        ``history`` is an optional list of prior ``{"role", "content"}`` message
        dicts (from ``ConversationHistory.to_messages_list()``).  When provided,
        these are inserted between the system prompt and the new user message so
        the model has full conversational context.  The conversation path does NOT
        use the exact-match cache because each turn is contextually unique.
