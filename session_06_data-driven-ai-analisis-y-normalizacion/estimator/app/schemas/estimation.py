...
class EstimationResult(BaseModel):
    ...
    def phases_sum_matches_total(self) -> "EstimationResult":
        ...
        # Override total to guarantee consistency rather than rejecting the
        # response — gpt-4o-mini reliably fails integer arithmetic on large sums.
...
class EstimationResponse(BaseModel):
    ...
    ``turn_meta`` is populated only by the conversational path and carries the
    same 13 fields emitted to the ``turn_observed`` structlog event. Callers
    that don't need it can ignore the field safely (it defaults to ``None``).
    """
