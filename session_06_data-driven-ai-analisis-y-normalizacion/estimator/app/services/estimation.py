...
def _count_tokens(text: str) -> int:
    """Rough token estimate: 4 chars ≈ 1 token (adequate for stress-test accounting)."""
...
class EstimationService:
    ...
    def estimate_conversational(
        ...
        *,
        ...
        # Capture pre-call state for turn_observed (after guardrail, before LLM).
        session.turn_count += 1
        turn_index = session.turn_count
        messages_in_window = len(session.history.messages)
        enriched_transcript_tokens = _count_tokens(transcript)
        ...
        # 8. Emit turn_observed event (observable via log aggregation) and
        #    embed the same 13 fields in the response for the stress runner.
        turn_meta = {
            "turn_index": turn_index,
            "session_id": session.session_id,
            "enriched_transcript_tokens": enriched_transcript_tokens,
            "context_total_tokens": meta.get("input_tokens", 0),
        ...
        log.info("turn_observed", **turn_meta)
