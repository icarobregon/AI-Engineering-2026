...
# --- Constructor (EstimationSemanticCache.__init__) -------------------------


def test_init_creates_search_index_with_correct_schema() -> None:
    """__init__ must call SearchIndex.from_dict, set_client, and create.

    SearchIndex is imported lazily inside __init__, so we patch the class at
    its source module (redisvl.index) before the local import executes.
    """
    from unittest.mock import MagicMock, patch
    ...
    with patch("redisvl.index.SearchIndex") as mock_si:
        mock_si.from_dict.return_value = fake_index
    ...
    mock_si.from_dict.assert_called_once()
    schema_arg = mock_si.from_dict.call_args[0][0]
    assert schema_arg["index"]["name"] == "test-index"
    fake_index.set_client.assert_called_once()
    fake_index.create.assert_called_once_with(overwrite=False)
    assert cache.threshold == 0.92


def test_init_tolerates_index_create_failure() -> None:
    """If SearchIndex.create raises (e.g. index already exists), __init__ must not raise."""
    from unittest.mock import MagicMock, patch

    from app.cache.semantic import EstimationSemanticCache

    fake_index = MagicMock()
    fake_index.create.side_effect = RuntimeError("Index already exists")
