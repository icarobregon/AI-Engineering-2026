...
# ---------------------------------------------------------------------------
# Error mapping: guardrail violation → 400, upstream failure → 502
# ---------------------------------------------------------------------------


class _RaisingEstimationService:
    """Service double that raises a configured exception on estimate()."""

    def __init__(self, exc: Exception) -> None:
        self._exc = exc

    def estimate(self, _request: EstimationRequest) -> EstimationResponse:
        raise self._exc


@pytest.fixture
def raising_service_factory():
    def _factory(exc: Exception):
        svc = _RaisingEstimationService(exc)
        app.dependency_overrides[get_estimation_service] = lambda: svc
        return svc
...
def test_guardrail_violation_returns_400(
    client: TestClient, raising_service_factory
) -> None:
    raising_service_factory(InputGuardrailViolation(reason="pii_detected", message="PII found in description"))
    response = client.post("/api/v1/estimate", json=VALID_PAYLOAD)
    assert response.status_code == 400
    detail = response.json()["detail"]
    assert detail["reason"] == "pii_detected"
...
def test_upstream_failure_returns_502(
    ...
    raising_service_factory(RuntimeError("LLM timed out"))
    response = client.post("/api/v1/estimate", json=VALID_PAYLOAD)
