...
def test_complete_structured_chat_forwards_messages(wrapper: LLMWrapper) -> None:
    ...
    model atomically. Now uses create_with_completion to expose token counts."""
    ...
    fake_raw = _fake_completion(model="gpt-4o-mini", input_tokens=120, output_tokens=40)
    with patch.object(
        wrapper._instructor.chat.completions,
        "create_with_completion",
        return_value=(expected, fake_raw),
    ) as mocked:
...
def test_complete_structured_chat_uses_anthropic_key_for_claude(wrapper: LLMWrapper) -> None:
    ...
    fake_raw = _fake_completion(model="claude-haiku-4-5-20251001")
    with patch.object(
        wrapper._instructor.chat.completions,
        "create_with_completion",
        return_value=(_Answer(text="x"), fake_raw),
    ) as mocked:
...
# ---------------------------------------------------------------------------
# Error paths — exception logging and re-raise
# ---------------------------------------------------------------------------


def test_complete_reraises_router_exception(wrapper: LLMWrapper) -> None:
    with patch.object(wrapper.router, "completion", side_effect=RuntimeError("api down")):
...
def test_complete_structured_chat_reraises_exception(wrapper: LLMWrapper) -> None:
    from pydantic import BaseModel
    ...
    with patch.object(
        wrapper._instructor.chat.completions,
        "create_with_completion",
        side_effect=ValueError("instructor failed"),
    ):
        with pytest.raises(ValueError, match="instructor failed"):
            wrapper.complete_structured_chat(
                messages=[{"role": "user", "content": "hi"}],
                response_model=_Answer,
            )
...
# ---------------------------------------------------------------------------
# complete_structured — non-conversational structured call
# ---------------------------------------------------------------------------


def test_complete_structured_happy_path(wrapper: LLMWrapper) -> None:
    from pydantic import BaseModel

    class _Answer(BaseModel):
        text: str

    expected = _Answer(text="structured ok")
    with patch.object(
...
def test_complete_structured_uses_anthropic_key_for_claude(wrapper: LLMWrapper) -> None:
    from pydantic import BaseModel
...
def test_complete_structured_reraises_exception(wrapper: LLMWrapper) -> None:
    from pydantic import BaseModel
    ...
    with patch.object(
        wrapper._instructor.chat.completions, "create", side_effect=RuntimeError("out")
    ):
