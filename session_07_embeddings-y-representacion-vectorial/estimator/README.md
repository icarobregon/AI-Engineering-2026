...
Pipeline mínimo que convierte presupuestos JSON en chunks (**un componente = un
chunk**) y los vectoriza con `text-embedding-3-small` (1536 dims). No persiste
nada: los vectores se devuelven por HTTP (pgvector llega en la Sesión 8). Todo el
código vive en `app/embedding_pipeline/`.

### Endpoint `POST /embeddings/ingest`

Recibe un array JSON de presupuestos y devuelve los chunks vectorizados más unas
estadísticas agregadas (`total_budgets`, `total_chunks`, `total_tokens`,
`estimated_cost_usd`). Visible en Swagger UI: http://localhost:8000/docs

```bash
# El fichero de muestra es directamente el body de la petición (un array).
curl -s -X POST http://localhost:8000/embeddings/ingest \
  -H "Content-Type: application/json" \
  --data @data/budgets_sample.json | jq '.stats'
```

Códigos: `200` OK, `422` validación (Pydantic), `503` si falta `OPENAI_API_KEY`,
`500` si falla la API de embeddings (detalle en logs, mensaje genérico al cliente).

### Script `scripts/compare.py`

Calcula la similitud coseno (a mano, sin numpy) entre dos textos, reutilizando el
mismo embedder del pipeline.

```bash
# Fuera del contenedor (recomendado; lee OPENAI_API_KEY del .env):
uv run python scripts/compare.py \
  --text-a "OAuth 2.0 authentication backend with JWT tokens" \
  --text-b "Authorization service using JSON Web Tokens"

# Dentro del contenedor: scripts/ NO está bind-mounted, así que hay que
# reconstruir la imagen antes (docker compose up --build) y luego:
docker compose exec estimator python scripts/compare.py \
  --text-a "..." --text-b "..."
```

La validación sobre los tres pares de referencia está en
[`app/embedding_pipeline/SANITY_CHECK.md`](app/embedding_pipeline/SANITY_CHECK.md).

> **Nota:** se añadió `tiktoken>=0.7` a `pyproject.toml`; reconstruye la imagen
> (`docker compose up --build`) o ejecuta `uv sync` en host tras hacer pull.
