...
@lru_cache
def get_chunker():
    """Structural chunker singleton (Session 7). Lazy import avoids a cycle:
    the pipeline modules import ``get_openai_client`` from here."""
...
@lru_cache
def get_embedder():
    """OpenAI embedder singleton (Session 7), or ``None`` if no API key —
    mirroring ``get_openai_client`` / ``get_semantic_cache``."""
