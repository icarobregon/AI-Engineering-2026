...
│   ├── llm/                #   LLMWrapper (LiteLLM + Instructor) + responses.py (Responses API,
│   │                       #   structured output strict) + runtime_config.py (overrides en Redis)
│   ├── prompts/            #   loader Jinja2 + plantillas versionadas (estimation/v1..v3, rag_estimation/v1, …)
...
│   └── persistence/        #   engine SQLAlchemy + repositories (jobs, mappings) + idempotency.py
│
├── domain/                 # contrato + conductor
│   ├── schemas/            #   EstimationRequest/Result/Response (contrato con el cliente)
...
│   ├── rag/                #   chunking/ + embedding/ + analysis/ + store/ + ingest_service.py +
│   │                       #   retriever.py + query_reformulator.py + context_assembler.py + generator.py
...
└── api/                    # transporte: routers finos, sin lógica de negocio
    ...
    ├── retrieval.py        #   POST /v1/retrieval/search (S9, con clave y rate limit)
    ├── estimate_rag.py     #   POST /v1/estimate/from-transcript (S9)
    ├── security.py         #   dos API keys independientes, compare_digest
    └── config.py           #   GET/PUT /api/v1/config/models (modelos en runtime)

rate_limit.py               # limiter compartido (por API key) + handler 429; vive con config/dependencies
```
...
- `estimate_from_transcript()` (S9) — idempotencia → **rag**.query_reformulator → **rag**.retriever
  (soft-fail: sin evidencia no se genera) → **rag**.context_assembler → **rag**.generator →
  validación de citaciones. Cinco etapas instrumentadas con `log_stage`.

RAG ya está en el camino de petición (S9) y entró **aquí**, como pasos del conductor: los cuatro
módulos de `generation/rag/` no se importan entre sí.
...
| Etapa del flujo RAG (reformulación, ensamblado de contexto, generación fundamentada) | `generation/rag/` (un módulo por etapa, sin importarse entre sí) |
| Autenticación de endpoint, rate limiting, idempotencia | `api/security.py`, `rate_limit.py`, `foundation/persistence/idempotency.py` |
...
- **Rutas HTTP**: `/api/v1/estimate`, `/sessions/*`, `/api/v1/ingestion/*`, `/embeddings/ingest`,
  `/search`, `/api/v1/config/models`, y desde la S9 `/v1/retrieval/search` y
  `/v1/estimate/from-transcript`.
  El cliente/frontend que las consuma depende de ellas y de la forma JSON de
  `EstimationResponse` / `ACBResponse` / `EstimateResponse`.
  Las dos rutas nuevas exigen `X-API-Key` con claves **distintas** entre sí: `/v1/retrieval` expone
  el corpus chunk a chunk y `/v1/estimate` cuesta dinero por llamada, así que se conceden, rotan y
  limitan por separado.
...
- `generation/rag/retriever.py` — recuperación semántica. **Implementado en el previo de la
  Sesión 8** (k-NN por distancia coseno); la **Sesión 9** añade `retrieve()` con umbral de calidad,
  filtros estructurales sobre JSONB y soft-fail (`low_confidence`).
...
- Integración del retriever en el conductor — **Implementado en la Sesión 9**
  (`estimate_from_transcript`), con la composición en `EstimationService` como manda la §7.
- Pendiente para la **Sesión 10**: reranking con cross-encoders, búsqueda híbrida (`tsvector` +
  vector), expansión/descomposición de consultas y routing multi-índice. Todo ello entra en
  `generation/rag/` y en el paso de retrieval del conductor; ni la augmentation ni la generación
