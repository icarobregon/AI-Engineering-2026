...
class Settings(BaseSettings):
    ...
    # --- Session 9 fields (RAG flow: query → retrieval → augmentation → generation) ---
    # Both stages go through the OpenAI Responses API (structured outputs with
    # strict schemas + reasoning effort), not through the LiteLLM wrapper: the
    # wrapper has no equivalent for either knob. See README, "Decisiones".
    RAG_REFORMULATION_MODEL: str = "gpt-5-mini"
    RAG_GENERATION_MODEL: str = "gpt-5"
    # Reasoning models replace ``temperature`` with reasoning effort.
    RAG_GENERATION_REASONING_EFFORT: Literal["minimal", "low", "medium", "high"] = "medium"

    # Retrieval defaults. The threshold is calibrated empirically against the
    # corpus (scripts/s09_calibrate_threshold.py), not chosen by intuition.
    RAG_TOP_K: int = 10
    RAG_DISTANCE_THRESHOLD: float = 0.6
    # Post-filtering widens the candidate pool by this factor before filtering.
    RAG_WIDE_K_FACTOR: int = 3

    # Augmentation. Total budget for one generation call; the assembler spends
    # 80% of it on context, reserving 15% for the answer and 5% for prompt
    # overhead (system message, instructions, structured query).
    RAG_TOKEN_BUDGET: int = 20_000
    # U-shaped reordering (lost in the middle). OFF by default: only worth it
    # when K grows to 15-20 and degradation is actually observed.
    RAG_CONTEXT_REORDER_U: bool = False

    # Idempotency of POST /v1/estimate/from-transcript, backed by Redis.
    RAG_IDEMPOTENCY_TTL: int = 86_400

    # Service layer. Two separate keys so the retrieval surface (the whole
    # historical corpus, chunk by chunk) and the estimation surface (expensive,
    # low volume) can be granted, rotated and rate-limited independently.
    # ``None`` disables the endpoint with a 503 rather than crashing at boot —
    # tests and local dev must not require secrets to import the app.
    RETRIEVAL_API_KEY: str | None = None
    ESTIMATE_API_KEY: str | None = None
    RATE_LIMIT_RETRIEVAL: str = "120/minute"
    RATE_LIMIT_ESTIMATE: str = "10/minute"
