...
from app.foundation.persistence.idempotency import IdempotencyStore
from app.generation.rag.context_assembler import ContextAssembler
from app.generation.rag.generator import EstimateGenerator
from app.generation.rag.ingest_service import RagIngestService
from app.generation.rag.query_reformulator import QueryReformulator
...
# --- Session 9: the RAG flow ------------------------------------------------


@lru_cache
def get_responses_client() -> ResponsesClient | None:
    """Structured-output client over the OpenAI Responses API.

    ``None`` without an OpenAI key, same contract as ``get_embedder``: the two
    LLM stages of the flow are OpenAI-only by design (strict schemas +
    reasoning effort), so no provider fallback applies here.
    """
    client = get_openai_client()
    if client is None:
        return None
    return ResponsesClient(client)


@lru_cache
def get_query_reformulator() -> QueryReformulator | None:
    client = get_responses_client()
    if client is None:
        return None
    return QueryReformulator(client, get_settings().RAG_REFORMULATION_MODEL)
...
def get_context_assembler() -> ContextAssembler:
    ...
    return ContextAssembler(
        ...
        reorder_u=settings.RAG_CONTEXT_REORDER_U,
    )


@lru_cache
def get_estimate_generator() -> EstimateGenerator | None:
    client = get_responses_client()
    if client is None:
        return None
    settings = get_settings()
...
def get_idempotency_store() -> IdempotencyStore:
    settings = get_settings()
    return IdempotencyStore.from_url(settings.REDIS_URL, ttl=settings.RAG_IDEMPOTENCY_TTL)
...
def get_estimation_service() -> EstimationService:
    ...
    return EstimationService(
        ...
        runtime_config=get_runtime_config(),
        # Session 9 — the RAG flow. All ``None``-able: without an OpenAI key
        # the conductor raises RagFlowUnavailable instead of half-running.
        query_reformulator=get_query_reformulator(),
