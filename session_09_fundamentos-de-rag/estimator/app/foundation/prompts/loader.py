...
def render_rag_estimation_prompt(
    *,
    ...
    """Render the prompts for the RAG estimation flow (Session 9).

    Takes plain strings rather than the RAG schemas on purpose: ``foundation``
    must not import from ``generation`` (ARCHITECTURE.md §3), so the caller
    serializes its own objects and this layer stays ignorant of them.

    ``invalid_ids`` drives the single retry after citation validation fails: the
    same template renders the corrective block, so the retry prompt cannot
    drift away from the original one.
    """
    context = {
        "context_block": context_block,
        "structured_query_json": structured_query_json,
        "invalid_ids": ", ".join(str(i) for i in invalid_ids) if invalid_ids else "",
    }
    system = _env.get_template(f"rag_estimation/{version}/system.j2").render(**context)
    user = _env.get_template(f"rag_estimation/{version}/user.j2").render(**context)
    return system, user
