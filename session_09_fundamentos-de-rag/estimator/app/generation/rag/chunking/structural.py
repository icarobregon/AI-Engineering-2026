...
def component_metadata(budget: Budget, component: BudgetComponent) -> dict:
    ...
    ``country`` was added in Session 9: the retriever filters geographically
    (an EstimationQuery carries a country) and the field existed in the source
    corpus but was never persisted, so the filter had no column to read.
    """
    return {
